<template>
  <div class="bg-white p-8 rounded-lg shadow-md">
    ...
    <div v-else class="space-y-6">
        <div v-for="job in jobs" :key="job.id" class="border rounded-lg p-6 flex justify-between items-center">
            <div>
                <p class="font-bold text-lg">{{ job.quote?.job_description || job.job_description }}</p>
                <p class="text-sm text-gray-500">Scheduled for: {{ new Date(job.scheduled_at || job.scheduled_date).toLocaleString() }}</p>
            </div>
